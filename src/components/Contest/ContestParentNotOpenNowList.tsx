...
function ContestParentNotOpenNowList(props: lessonListProps): JSX.Element {
    ...
    const routeChange = (contest_parent: IContestParentX) =>{ 
        localStorage.removeItem("contest_id")
        localStorage.setItem('contest_id', contest_parent.id.toString())
        localStorage.removeItem("contest_name")
        localStorage.setItem('contest_name', contest_parent.name)
        ...
        localStorage.setItem('art_age_contest', contest_parent.art_age_name)
        localStorage.removeItem("registration_time")
        localStorage.setItem('registration_time', contest_parent.registration_time)
    ...
    useEffect(() => {
        let x = (contest_parents.contest_not_open_now.length - contest_parents.contest_not_open_now.length % 10) /10;
        if (x === 0) {
            setElement(contest_parents.contest_not_open_now)
        }
        else {
            setElement(contest_parents.contest_not_open_now.slice(0,10))
        }
        
         setTotalPage((x+1))
    }, [contest_parents.contest_not_open_now])
    ...
    function handlePagination(count: number) {
        console.log(count)
        if (count === totalPage) {
            setElement(contest_parents.contest_not_open_now.slice(count*10))
        }
        else {
            setElement(contest_parents.contest_not_open_now.slice(count*10,count*10 + 10))
        }
    }
    ...
    function handleFilter() {
        if (filter2 == 0) {
            if (filter == 0 && filter1 == 0) {
                let k = contest_parents.contest_not_open_now;
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_age_id == filter1)
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_type_id == filter)
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_type_id == filter && ele.art_age_id == filter1)
            ...
            if (filter == 0 && filter1 == 0) {
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.student_id == filter2);
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_age_id == filter1 && ele.student_id == filter2)
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_type_id == filter && ele.student_id == filter2)
                ...
                let k = contest_parents.contest_not_open_now.filter((ele, idx) => ele.art_type_id == filter && ele.art_age_id == filter1 && ele.student_id == filter2)
...
export default ContestParentNotOpenNowList;
