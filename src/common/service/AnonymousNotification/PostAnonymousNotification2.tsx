...
export function postAnonymousNotification2(data: any, idx: any) {
    var bearer = 'Bearer ' + localStorage.getItem("access_token");
    return (dispatch: any) => {
        dispatch(fetchDataRequest());
        fetch(
            `${process.env.REACT_APP_API_URL}/sendEmail/student`, {
            method: "POST",
            headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    if (response.status === 403) {
                        ...
                        dispatch(postAnonymousNotification2(data, idx))
                    }
                    else {
