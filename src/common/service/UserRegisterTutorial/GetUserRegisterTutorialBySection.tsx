...
export function getUserRegisterTutorialBySection(dispatch: any, section_id: number) {
    var bearer = 'Bearer ' + localStorage.getItem("access_token");
    
    return  fetch(
                `${process.env.REACT_APP_API_URL}/user-register-tutorial/section/${section_id}`, {
                    method: "GET",
                    headers: {
            ...
            .then( response => {
                if (!response.ok) {
                    if (response.status === 403) {
                        ...
                        dispatch(getUserRegisterTutorialBySection(dispatch, section_id))
                    }
                    else {
