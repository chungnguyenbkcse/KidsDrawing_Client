...
export function getExerciseSubmissionBySectionAndParent(dispatch: any, section_id: number, parent_id: number) {
    var bearer = 'Bearer ' + localStorage.getItem("access_token");
    return  fetch(
                `${process.env.REACT_APP_API_URL}/exercise-submission/section-parent/${section_id}/${parent_id}`, {
                    method: "GET",
                    headers: {
            ...
            .then( response => {
                if (!response.ok) {
                    if (response.status === 403) {
                        ...
                        dispatch(getExerciseSubmissionBySectionAndParent(dispatch, section_id, parent_id))
                    }
                    else {
